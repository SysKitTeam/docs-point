---
description: >-
  This article explains how to resolve the Inactive Workspaces task in Syskit
  Point.
---
...
* [Inactive Workspaces Emails](inactive-workspaces.md#inactive-workspaces-e-mails)
* [Inactive Workspaces Tasks](inactive-workspaces.md#inactive-workspaces-tasks)
* [Inactive Workspaces Actions](inactive-workspaces.md#inactive-workspaces-actions)
...
{% hint style="info" %}
**Hint!**   On the Sites overview screen and the Microsoft Teams & Groups overview screen, you **run the Restore action on workspaces in bulk** by selecting multiple workspaces and clicking the wanted action from the side panel.
{% endhint %}
...
* **Select the period (1)** for which the workspace should be kept
  * The selected period cannot exceed the defined number of days after for keeping a workspace, which can be changed in the [Inactive Workspaces policy](../../governance-and-automation/automated-workflows/inactive-workspaces-admin.md) settings.
...
{% hint style="warning" %}
**Please note!**   The Keep action can be executed for inactive workspaces only!
{% endhint %}
...
* **If the Remove Access option was enabled and depending on the parameters set by the admin, access to the workspace is removed for**:
  * **Members**; all members are removed when a workspace is archived   \* **Owners**; all owners are removed when a workspace is archived; when this option is set up, **at least 1 owner will still exist for all workspaces** as set by the admin.
...
{% hint style="info" %}
**Hint!**   There is no time limit for **performing the Restore action on the Archived workspace**.
{% endhint %}
