...
* [Enable the Syskit Point Collaborators role.](../../configuration/enable-role-based-access.md)
...
The purpose of this is to assist Syskit Point admins by reminding them that there are potential issues in their Microsoft 365 environment every time they open Syskit Point. 
...
* **You can also set up policy rules to have policies automatically applied to current and new workspaces.**
...
By clicking on the names of each policy, you can **find an article with information on enabling and managing** all the mentioned policy types. 
...
* **Modifying detection options for policies is applied within 24 hours**. For example, changing the minimum number of required owners would take up to 24 hours to apply.
...
* [Resolve Tenant Storage vulnerabilities](../../point-collaborators/resolve-governance-tasks/tenant-storage-limit.md)