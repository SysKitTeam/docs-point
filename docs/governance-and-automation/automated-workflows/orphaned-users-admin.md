...
When a **user account is deleted from Microsoft Entra ID, or has sign-in blocked, their entry typically remains cached in the SharePoint UserInfo list** on every site they visited. This redundant entry becomes a problem when a new account is later created with the same username (UPN). Microsoft 365 assigns the new account a new internal ID, but the UserInfo list still holds the old one, causing a mismatch.
...
**The root cause is a UserInfo list mismatch.** When an account is deleted and re-created with the same UPN, the new account receives a different ID. SharePoint and OneDrive still reference the old ID in the UserInfo list, which makes access checks fail.
