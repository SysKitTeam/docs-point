...
**When external sharing settings become more restrictive, SharePoint blocks existing Anyone links, but doesn't delete them.** These links were valid when created, but once your settings became more restrictive, **they no longer comply with your external sharing policy and should be removed**. Deleting these links removes the risk permanently. Unlike tightened settings that can be reverted, a deleted link cannot be reactivated.  
