...
To successfully enable webhooks, you need to perform the following steps:

* **Register endpoint** to which Syskit Point will send notifications.
* **Obtain the signature key** to verify the event was sent by Syskit Point.
...
* **Signature**: Contains the message signature created by using the hashing algorithm provided in the Algorithm header using a signature key.
...
Below is the code example that you can use to create the signature, which can then be compared with the one received in the event object to verify it was sent from Syskit Point.
